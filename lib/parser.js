...
module.exports = function(cmd) {
  ...
  if (cmd.path) {
    program.loom.paths.push(cmd.path);
  }
  if (fs.existsSync('loom')) {
    program.loom.paths.push('loom');
  }
...
function parseArgsAndParams(program) {
  var i, arg, split;
  // -1 to skip the "command" object in program.args
  for (i = 0; i < program.args.length - 1; i += 1) {
    arg = program.args[i];
    split = arg.split(':');
    if (split.length == 2) {
